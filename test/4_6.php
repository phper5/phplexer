...
$block = Parser::rule(\diandi\stone\ast\BlockStmnt::class)
    ->sep("{")
    ->option($statement0)
    ->repeat(Parser::rule()->sep(";", new EOFToken())
                            ->option($statement0))
    ->sep("}");
$simple = Parser::rule(PrimaryExpr::class)->ast($expr);
$statement = $statement0->or(
                            Parser::rule(\diandi\stone\ast\IfStmnt::class)->sep("if")
                                                                                ->ast($expr)
                                                                                ->ast($block)
                                                                                ->option(Parser::rule()->sep("else")
                                                                                                        ->ast($block)),
                            Parser::rule(\diandi\stone\ast\WhileStmnt::class)->sep("while")
                                                                                    ->ast($expr)
                                                                                    ->ast($block),
                            $simple);
